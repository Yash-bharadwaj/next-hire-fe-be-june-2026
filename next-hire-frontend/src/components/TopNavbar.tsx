...
export function TopNavbar() {
  ...
  const isDetailPage = () => {
    ...
    return (
      ...
      (path.includes("/dashboard/candidates/") &&
        path.split("/").length === 4) ||
      (path.includes("/dashboard/submissions/") &&
        path.split("/").length === 4) ||
      (path.includes("/dashboard/interviews/") &&
        path.split("/").length === 4) ||
      (path.includes("/dashboard/placements/") &&
        path.split("/").length === 4) ||
      (path.includes("/dashboard/business-partners/") &&
        path.split("/").length === 4)
    );
  };
  ...
  const fullName = [user?.first_name, user?.last_name]
    ...
    .join(" ")
    .trim();
  const displayName = fullName || user?.email?.split("@")[0] || "Team Member";
  const userEmail = user?.email || "user@thenexthire.com";
  const userInitials = fullName
    ? fullName
        ...
        .substring(0, 2)
        .toUpperCase()
    : user?.email?.charAt(0)?.toUpperCase() || "TN";
    ...
    <div className="flex items-center justify-end flex-1 px-4">
      ...
      <div className="flex items-center gap-3">
        ...
        {user?.role !== "client" && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-60 bg-white">
              ...
              <DropdownMenuItem
                className="p-3"
                onClick={() => navigate("/dashboard/settings?tab=ai-workbench")}
              >
                ...
                <DropdownMenuItem
                  ...
                  onClick={() => {
                    window.dispatchEvent(
                      new CustomEvent("openPersonalizationSettings")
                    );
                  }}
                >
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="w-80 p-0 bg-white border-green-200 shadow-2xl z-50"
          >
            {/* User Header */}
            <div className="p-4 border-b border-green-100 bg-gradient-to-r from-green-50 to-green-100/50">
              <div className="flex items-center gap-3">
                ...
                <div
                  className={`${statusBadgeClass} text-white text-xs px-2 py-1 rounded font-medium capitalize`}
                >
                  {statusLabel}
                </div>
              </div>
                ...
                <DropdownMenuSubContent className="w-56 bg-white border-green-200 shadow-xl">
                  ...
                  <DropdownMenuItem className="p-0 hover:bg-green-50">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/dashboard/users/${user?.id || "1"}?tab=preferences`
                        )
                      }
                    >
                      <Settings className="w-4 h-4 text-green-600" />
                  ...
                  <DropdownMenuItem className="p-0 hover:bg-green-50">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/dashboard/users/${user?.id || "1"}?tab=defaults`
                        )
                      }
                    >
                  ...
                  <DropdownMenuItem className="p-0 hover:bg-green-50">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/dashboard/users/${user?.id || "1"}?tab=integrations`
                        )
                      }
                    >
                  ...
                  <DropdownMenuItem className="p-0 hover:bg-green-50">
                    <button
                      ...
                      onClick={() =>
                        navigate(
                          `/dashboard/users/${user?.id || "1"}?tab=credits`
                        )
                      }
                    >
