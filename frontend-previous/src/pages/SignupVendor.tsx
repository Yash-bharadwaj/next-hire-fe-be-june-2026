...
export default function SignupVendor() {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", "Create your vendor account on TNH to collaborate with clients and submit candidates.");
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: `Welcome, ${contactName || "Vendor"}!`, description: "Your account has been created." });
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50/30 to-green-100/50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-8 relative z-10">
        <div className="text-center">
          ...
          <p className="text-green-600/70 font-medium">Collaborate with clients and manage submissions</p>
        </div>
        ...
        <Card className="border-0 shadow-2xl shadow-green-500/10 bg-white/95 backdrop-blur-xl relative overflow-hidden">
          ...
          <CardHeader className="space-y-1 pb-6 relative z-10">
            <CardTitle className="text-2xl text-center bg-gradient-to-r from-green-700 to-emerald-600 bg-clip-text text-transparent">Sign Up (Vendor)</CardTitle>
            <CardDescription className="text-center text-green-600/70">Start collaborating with your clients</CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="company" placeholder="Acme Staffing" value={companyName} onChange={(e) => setCompanyName(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="name" className="text-green-700 font-medium">Contact name</Label>
                <div className="relative group">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />
                  <Input id="name" placeholder="Jane Recruiter" value={contactName} onChange={(e) => setContactName(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="email" type="email" placeholder="you@acme.com" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" required />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="password" type="password" placeholder="Create a password" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" required />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="confirm" className="text-green-700 font-medium">Confirm password</Label>
                <div className="relative group">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />
                  <Input id="confirm" type="password" placeholder="Repeat your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" required />
                </div>
              </div>

              <label className="flex items-center gap-3 text-sm text-green-700">
                <input type="checkbox" checked={accepted} onChange={(e) => setAccepted(e.target.checked)} className="w-4 h-4 text-green-600 border-green-300 rounded focus:ring-green-500 focus:ring-2" />
                I agree to the Terms and Privacy Policy
              </label>

              <Button type="submit" disabled={isLoading} className="w-full h-12 bg-gradient-to-r from-green-600 via-green-500 to-emerald-500 hover:from-green-700 hover:via-green-600 hover:to-emerald-600 text-white font-semibold rounded-xl shadow-lg shadow-green-500/25 transition-all duration-300">
                {isLoading ? "Creating account..." : "Create account"}
              </Button>
            </form>

            <div className="mt-6 text-center text-sm text-green-700">
              Already have an account? <Link to="/auth/login" className="font-semibold hover:underline">Sign in</Link>
            </div>
          </CardContent>
        </Card>

        <div className="text-center text-sm text-green-700">
          Are you a Candidate? <Link to="/auth/signup-candidate" className="font-semibold hover:underline">Sign up here</Link>
        </div>
      </div>
    </div>
  );
