...
export default function SignupCandidate() {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute("content", "Create your candidate account on TNH to track applications and interviews.");
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Welcome, " + (fullName || "Candidate") + "!", description: "Your account has been created." });
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 via-emerald-50/30 to-green-100/50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-8 relative z-10">
        <div className="text-center">
          ...
          <p className="text-green-600/70 font-medium">Apply faster and track your progress</p>
        </div>
        ...
        <Card className="border-0 shadow-2xl shadow-green-500/10 bg-white/95 backdrop-blur-xl relative overflow-hidden">
          ...
          <CardHeader className="space-y-1 pb-6 relative z-10">
            <CardTitle className="text-2xl text-center bg-gradient-to-r from-green-700 to-emerald-600 bg-clip-text text-transparent">Sign Up (Candidate)</CardTitle>
            <CardDescription className="text-center text-green-600/70">It only takes a minute</CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="name" placeholder="Jane Doe" value={fullName} onChange={(e) => setFullName(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="email" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" required />
                </div>
              </div>

              <div className="space-y-2">
                <Label htmlFor="phone" className="text-green-700 font-medium">Phone (optional)</Label>
                <div className="relative group">
                  <Phone className="absolute left-3 top-1/2 -translate-y-1/2 text-green-500 w-5 h-5" />
                  <Input id="phone" placeholder="+1 (555) 000-0000" value={phone} onChange={(e) => setPhone(e.target.value)} className="pl-11 h-12 border-green-200/50 focus:border-green-400 focus:ring-green-400/20 bg-white/70" />
                </div>
              </div>
        ...
        <div className="text-center text-sm text-green-700">
          Are you a Vendor? <Link to="/auth/signup-vendor" className="font-semibold hover:underline">Sign up here</Link>
        </div>
      </div>
    </div>
  );
